...
export const TablePagingControl = ({ numOfPage }: any) => {
          ...
          return (
            <button
              className={`border px-3 py-1 ${
                index == curPage ? "bg-sky-100 text-sky-500" : ""
              }`}
              onClick={() => {
  ...
  return (
    <TableContext.Consumer>
      {({ totalPage, curPage }) => (
        <div className={"p-5 bg-slate-100 float-right"}>
      ...
      {({ numOfRow, numOfCol, widthCol }) => {
        return (
          <div>
            <div
              className={`flex p-4 ${
                isHeader ? "bg-blue-200" : "bg-slate-100"
              }`}
            >
              {[...Array(numOfCol)].map((value, index) => {
                return (
                  <div style={{ width: `${widthCol}px` }}>
                    <span className={`${isHeader ? "font-semibold" : ""}`}>
                      {data[Object.keys(data)[index] as keyof Object]}
                    </span>
                  </div>
                );
              })}
              {!isHeader && (
                <div className={"flex"}>
                  ...
                  <FiMoreVertical
                    className={"ml-5 text-2xl text-gray-400 cursor-pointer"}
                  />
                </div>
              )}
            </div>
