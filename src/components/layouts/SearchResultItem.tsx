...
export const SearchResultItem = ({ data }: SearchResultItemProp) => {
  ...
  useEffect(() => {
    axiosClient.get<PInScore>(`service/${data?._id}/scores`).then((res) => {
  ...
  return (
    <div
      className={`flex p-2 m-2 hover:bg-blue-200 transition-all duration-300 cursor-pointer rounded`}
      onClick={() => {
      ...
      <div className={"ml-4"}>
        ...
        <div>
          <p className={"italic text-sm"}>
            Đánh giá: {data?.rankingScore ?? score.toFixed(2)}/10.0
          </p>
        </div>
      </div>
    </div>
