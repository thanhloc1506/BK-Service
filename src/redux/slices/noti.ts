...
export const getNoti = createAsyncThunk("/noti", async () => {
  return axiosClient
...
export const readAllNoti = createAsyncThunk("/read-all-noti", async () => {
...
const notiSlice = createSlice({
  ...
  reducers: {
    addNewNoti: function (
      state: State,
      action: PayloadAction<PInNotification.Notification>
    ) {
  ...
  extraReducers: {
    [getNoti.pending.toString()]: (state, _action) => {
      state.notiLoading = true;
    },
    [getNoti.fulfilled.toString()]: (
      state: State,
      action: PayloadAction<PInNotification.Data>
    ) => {
